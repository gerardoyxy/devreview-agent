...
En Windows, usa `target\debug\nudgethis.exe demo`. La demo crea un repositorio temporal,
...
2. Desde la raíz del repositorio de tu aplicación, ejecuta `/ruta/absoluta/nudgethis init`.
3. Revisa `nudgethis.toml`, registra agentes instalados y autenticados, configura validaciones y confirma los archivos en Git.
4. Inicia tu aplicación y, en otra terminal, `/ruta/absoluta/nudgethis start`.
...
`nudgethis.config.mjs`: sus valores pasan a TOML. La primera apertura de SQLite anterior crea
...
El chat de NudgeThis pertenece a esa tarea; no se conecta a conversaciones externas ya abiertas.
