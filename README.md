...
npm ci
npm run build
./target/debug/devreview --help
```

Windows: `target\debug\devreview.exe --help`. For a release executable, run
`cargo build --release --locked -p devreview` after building the frontend.
...
# Review detected setup and validation commands in devreview.toml.
/absolute/path/to/devreview doctor
/absolute/path/to/devreview start --no-execution
```
...
```text
devreview init                  devreview doctor
devreview start [--port 7331] [--no-execution]
devreview stop                  devreview status
devreview tasks                 devreview task QA-1
devreview apply QA-1             devreview undo QA-1
devreview reject QA-1
