...
tokio::task_local! { static SINK: std::sync::Arc<dyn Fn(serde_json::Value) + Send + Sync>; }

fn emit(event: Event<'_>) -> Result<()> {
    let mut value = serde_json::to_value(event)?;
    value["protocolVersion"] = protocol::VERSION.into();
    if SINK.try_with(|sink| sink(value.clone())).is_ok() {
        return Ok(());
    }
...
pub async fn run_stdio() {
    let result = async {
        ...
        execute(request, cancellation_signal()?).await
    }
    .await;
...
pub async fn run(
    request: RunRequest,
    sink: std::sync::Arc<dyn Fn(serde_json::Value) + Send + Sync>,
    cancellation: impl std::future::Future<Output = ()>,
) -> Result<()> {
    SINK.scope(sink, execute(request, cancellation)).await
}
