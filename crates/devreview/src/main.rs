...
async fn run() -> Result<()> {
    let cli = Cli::parse();
    match cli.command {
        Command::AgentRun => devreview_agent_runtime::run_stdio().await,
...
fn init(root: &std::path::Path) -> Result<()> {
    ...
    println!(
        "Created devreview.toml. Review agents and validation commands, then commit configuration before starting."
    );
    Ok(())
}
