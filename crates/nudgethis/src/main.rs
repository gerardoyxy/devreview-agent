...
fn init(root: &std::path::Path) -> Result<()> {
    ...
    if !text
        .lines()
        .any(|l| matches!(l.trim(), ".nudgethis/" | "/.nudgethis/"))
    {
        let mut file = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
...
async fn demo(port: u16) -> Result<()> {
    ...
    let result=async {
        std::fs::write(root.join(".gitignore"),".nudgethis/\n")?; std::fs::write(root.join("button.css"),".button { color: red; border-radius: 4px; }\n")?;
        let repo=git::Repository::new(root.clone(),root.join(".nudgethis"));
        repo.git(&["init","-b","main"],&root,"").await?; repo.git(&["add","."],&root,"").await?;
        repo.git(&["-c","user.name=NudgeThis Demo","-c","user.email=demo@example.invalid","-c","commit.gpgsign=false","commit","-m","Demo fixture"],&root,"").await?;
