...
| Save approved code | Guided local commits with selected corrections, editable names, explicit authors and saved-version history. API and browser checks cover real Git commits, preserved unrelated staging, stale reviews and mobile controls. | Available in current source after `0.4.0-alpha.1`. Hooked repositories and mixed edits within a selected file use a Git client. Ambiguous interruptions require manual recovery. |
| Understand branches and share work | No-repository guide, reviewed first version, create/switch/continue, refreshed branch indicators, explicit GitHub account and destination, reviewed publication, proposals and gated merge. Local Git, simulated GitHub API and Chromium tests cover the workflow. | Source after `0.4.0-alpha.1`. Requires Git for changes and GitHub CLI for publishing. No live-service or organization-policy certification, conflict resolver, force push, automatic local-main update or deployment guarantee. |
