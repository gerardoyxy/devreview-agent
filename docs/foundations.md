...
| --- | --- | --- |
| Start or open a project | Portable launcher, local project library, goal diagnosis, reviewed HTML/CSS, Astro and React starters, and owned preview controls. `init` preserves configuration; `doctor` inspects prerequisites without executing them. | Agent setup still requires a terminal. Managed previews support the maintained starters and npm-based Astro/Vite projects; other frameworks need manual development-server and overlay setup. Detection does not prove a framework integration. |
| Explain a change without UI vocabulary | Configurable mouse/keyboard selection, single or multiple click/tap picking, area selection, group review, parent/child/sibling navigation, free-text requests, general/backend tasks and optional file references. Browser checks exercise selection and draft creation. | No direct drag-to-resize editor or verified DOM-to-source mapping. Ambiguous selectors require reselection. |
...
| Save approved code | Guided local commits with selected corrections, editable names, explicit authors and saved-version history. API and browser checks cover real Git commits, preserved unrelated staging, stale reviews and mobile controls. | Hooked repositories and mixed edits within a selected file use a Git client. Ambiguous interruptions require manual recovery. |
| Understand branches and share work | No-repository guide, reviewed first version, create/switch/continue, refreshed branch indicators, explicit GitHub account and destination, reviewed publication, proposals and gated merge. Local Git, simulated GitHub API and Chromium tests cover the workflow. | Requires Git for changes and GitHub CLI for publishing. No live-service or organization-policy certification, conflict resolver, force push, automatic local-main update or deployment guarantee. |
| Reuse a personal style | Visual branching builder, saved profiles, scoped rules, portable exports and explicit/default style context. Rust checks suggestion evidence; API/browser checks cover persistence, stale saves and scoped drafts. | Suggestions recognize a bounded subset of CSS value corrections, not utility classes or semantic intent. Rules require user acceptance; agent compliance and the resulting visual change are not certified by these tests. |
...
1. Extend guided setup to agent configuration and more existing-project integrations:
   prerequisites, running origin, overlay, saved draft and configured validation.
