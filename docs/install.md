...
3. Extract into a directory you own. The archive contains `devreview` or `devreview.exe`,
...
```sh
/path/to/devreview init
/path/to/devreview doctor
/path/to/devreview start --no-execution
```

Use a repository with at least one commit. `init` adds `.devreview/` to `.gitignore` and
suggests commands from project metadata without executing them. Review `devreview.toml`.
...
path to `devreview.exe` in PowerShell.
...
Remove the development overlay integration from your application. Keep `.devreview/` if
...
cargo build --release --locked -p devreview
