...
The landing is a static website. Keep the code on `gerardoyxy/devreview-agent`
...
2. Authorize the GitHub integration for **gerardoyxy**, using **Only select
   repositories** and selecting **devreview-agent**. Keep `hejoirsys` outside
   this installation.
...
under a project path such as `/devreview-agent/`.
