...
2. En el checkout de DevReview: `npm ci`, `npm run build`. El resultado es `target/debug/devreview`
   o `target/debug/devreview.exe`. Para release, después compila con `cargo build --release --locked`.
3. En la aplicación, ejecuta el nuevo binario con `init`. No sobrescribe `devreview.toml` existente.
4. Traslada los valores de `devreview.config.mjs` a `devreview.toml` y revisa el resultado.
5. Confirma la configuración y `.gitignore`, luego inicia con `devreview start`.
...
| `DEVREVIEW_RUNTIME` | Ya no se usa: el servidor incorpora la biblioteca Rust |
| `node .../packages/cli/src/index.js start` | `/ruta/al/binario/devreview start` |
...
Se conserva `.devreview/token` y se abre `.devreview/tasks.sqlite`. Si la base anterior tiene
