...
Protected paths such as `.git`, `.nudgethis` and `.env` are refused. Changed symlinks
...
Use Ctrl+C or `nudgethis stop` for an orderly shutdown. Process groups/jobs stop active
...
`nudgethis start --no-execution` to inspect the state without running them.

If a crash left `.nudgethis/server.lock`, verify that the old service and its children
have stopped before removing that lock. Never run two versions against the same state.
Back up `.nudgethis/` only while the service is stopped; do not mix unrelated SQLite WAL files.
