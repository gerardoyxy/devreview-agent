...
const binary = fileURLToPath(new URL(`../../../target/debug/nudgethis-agent-runtime${process.platform === 'win32' ? '.exe' : ''}`, import.meta.url));

/** Temporary Node-to-Rust bridge. All provider protocols/processes live in Rust. */
export class NativeAgent {
  constructor({ id = 'codex', label = id, transport = 'codex', command, args = [], timeout = 600000, model, runtime = process.env.NUDGETHIS_RUNTIME || binary } = {}) {
  ...
  async run({ task, cwd, signal, onMessage = () => {} }) {
    ...
    try {
      ...
      if (error.code === 'ENOENT') throw new Error('Native agent runtime is missing. Run cargo build, or configure NUDGETHIS_RUNTIME with the compiled executable path.');
      throw error;
    }
    consume(buffer);
