...
async function main() {
  if (command === 'init') {
    ...
    } catch (error) { if (error.code !== 'EEXIST') throw error; console.log('Keeping existing devreview.config.mjs'); }
    let ignore = '';
    try { ignore = await readFile(path.join(root, '.gitignore'), 'utf8'); } catch (error) { if (error.code !== 'ENOENT') throw error; }
    if (!ignore.split(/\r?\n/).some(line => ['.devreview/', '/.devreview/'].includes(line))) await appendFile(path.join(root, '.gitignore'), '\n# Local QA state (credentials, database, worktrees)\n.devreview/\n');
    console.log('Initialized DevReview. Configure validation, then commit the config and .gitignore before starting QA.'); return;
  }
  if (command === 'start') {
    const app = await startServer({ root });
    console.log(`\nDevReview Agent · ${app.queue.agent.name}\nDashboard: ${app.url}/#token=${app.token}\nOverlay module: ${app.url}/overlay.js\nListening only on 127.0.0.1. Press Ctrl+C to stop.\n`);
    if (!app.config.commands.length) console.log('No validation commands configured. Add them to devreview.config.mjs.');
    let stopping = false;
  ...
  if (['status', 'tasks', 'task', 'apply', 'reject', 'retry', 'cancel'].includes(command)) {
    ...
    const token = (await readFile(path.join(root, '.devreview/token'), 'utf8')).trim();
