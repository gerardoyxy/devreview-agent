...
export async function createCore({ root, config, agent } = {}) {
  ...
  const stateDir = path.join(root, '.nudgethis');
  await mkdir(stateDir, { recursive: true, mode: 0o700 });
  assert(await realpath(stateDir) === stateDir, '.nudgethis must be a real directory inside the repository');
  const lockPath = path.join(stateDir, 'server.lock');
  let lock;
  try { lock = await open(lockPath, 'wx', 0o600); }
  catch (error) {
    if (error.code !== 'EEXIST') throw error;
    throw new Error('A server lock exists. Stop the other NudgeThis server; if it crashed, confirm it is stopped before removing .nudgethis/server.lock.');
  }
  ...
  try {
    ...
    try { await repository.git(['check-ignore', '--quiet', '--no-index', '.nudgethis/token']); }
    catch { throw new Error('Ignore .nudgethis/ before starting the server. Run nudgethis init, then commit the configuration.'); }
