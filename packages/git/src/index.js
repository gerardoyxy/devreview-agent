...
export class Repository {
  ...
  async inspect() {
    ...
    assert(await realpath(root) === await realpath(this.root), 'Start DevReview at the repository root');
  ...
  async collect(task) {
    ...
    await this.git(['add', '-A', '--', '.', ':!.devreview'], cwd);
    ...
    for (let i = 0; i < parts.length - 1; i += 2) {
      ...
      assert(!file.split('/').some(segment => ['.git', '.devreview'].includes(segment)), 'Protected path in patch', 409);
