...
export function elementContext(element, { captureDom = false } = {}) {
  ...
  const context = {
    ...
    source: element.dataset.nudgethisSource || '',
...
export const NudgeThis = {
  init({ server = 'http://127.0.0.1:7331', token, enabled = false, modifier = 'alt', captureDom = false } = {}) {
    ...
    if (!token) throw new Error('Copy the local token from the NudgeThis dashboard');
    ...
    <a class="launcher" target="_blank" rel="noopener"><span class="dot"></span><span class="label">NudgeThis · connecting</span></a>`;
    ...
    void watchTasks(server, token, update, controller.signal, connected => {
      $('.label').textContent = connected ? `NudgeThis · ${modifier === 'alt' ? 'Alt + ' : ''}right-click` : 'NudgeThis · disconnected';
