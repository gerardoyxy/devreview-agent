...
export function elementContext(element: Element, { captureDom = false } = {}): ElementContext {
  ...
  const context: ElementContext = {
    ...
    sourceVerified: false, source: element.getAttribute('data-devreview-source') || '',
...
export function resolveElement(context: ElementContext): Element | undefined {
  ...
  try {
    ...
    if (target.closest('[data-devreview-overlay]') || target.tagName.toLowerCase() !== context.tagName) return;
