  ...
  init({ server = 'http://127.0.0.1:7331', token, enabled = false, modifier = 'alt', captureDom = false }: OverlayOptions = {}) {
    ...
    shadow.innerHTML = `<style>${themeDefaults}
      ...
      *{box-sizing:border-box}button,a,textarea,select{font:inherit}button,a{cursor:pointer}
      ...
      .save{background:var(--dr-accent);color:var(--dr-onAccent)}.dot,.outline{border-color:var(--dr-accent)}.dot{background:var(--dr-accent)}.outline{background:color-mix(in srgb,var(--dr-accent) 10%,transparent)}.error,.review-error{color:var(--dr-danger)}.target,.marker{color:var(--dr-accent)}.review-dialog::backdrop{background:color-mix(in srgb,var(--dr-backdrop) 65%,transparent);backdrop-filter:none}.review-logo{background:var(--dr-accent);color:var(--dr-onAccent)}.review-task[aria-current=true]{background:var(--dr-accentSoft)}.appearance-button{border:1px solid var(--dr-border);border-radius:var(--dr-radius);background:var(--dr-surface);color:var(--dr-text);padding:6px 9px;font-size:12px}.head strong,.review-brand{font-family:var(--dr-font-heading)}button:focus-visible,textarea:focus-visible,select:focus-visible,a:focus-visible{outline:2px solid var(--dr-accent);outline-offset:3px}
</style>
