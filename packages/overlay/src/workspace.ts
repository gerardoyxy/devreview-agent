...
function shell(mount: HTMLElement | ShadowRoot, title: string) {
...
export function createDiagnostics(mount: HTMLElement | ShadowRoot, api: Api) {
  ...
  const open = async () => {
    ...
    try {
      ...
      row('Configuration', report.configurationExists ? 'devreview.toml found' : 'Run devreview init');
      ...
      content.append(element('p', 'Review devreview.toml to change commands or origins, then restart the server. The configuration belongs to your repository.'));
