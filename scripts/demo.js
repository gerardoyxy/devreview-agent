...
const root = await mkdtemp(path.join(tmpdir(), 'devreview-demo-'));
...
for (const args of [['init', '-b', 'main'], ['add', '.'], ['-c', 'user.name=DevReview Demo', '-c', 'user.email=demo@example.invalid', '-c', 'commit.gpgsign=false', 'commit', '-m', 'Demo app']]) {
  const result = await run('git', args, { cwd: root }); if (result.code) throw new Error(result.stderr);
}
const demoAgent = { name: 'demo (predefined button fix)', async run({ cwd, signal }) {
  await new Promise(resolve => setTimeout(resolve, 900)); signal.throwIfAborted();
  await writeFile(path.join(cwd, 'button.css'), after);
  return { output: 'Demo adapter: aligned the button to the right and added a mobile full-width rule. No external model was called.' };
} };
let app;
