...
export const binary = process.env.NUDGETHIS_TEST_BINARY || fileURLToPath(new URL(`../target/debug/devreview${process.platform === 'win32' ? '.exe' : ''}`, import.meta.url));
...
export async function application({ origins = [] } = {}) {
  ...
  await writeFile(path.join(root, 'devreview.toml'), '[execution]\nenabled = false\n' + (origins.length ? `\n[server]\nallowedOrigins = ${JSON.stringify(origins)}\n` : ''));
  git('add', '--', '.');
  ...
  const start = async () => {
    ...
    for (let n = 0; n < 200; n++) {
      ...
      if (match) {
        ...
        token = (await readFile(path.join(root, '.devreview/token'), 'utf8')).trim();
