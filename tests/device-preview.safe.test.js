...
test('device preview rejects unsafe requests and reports a missing browser without launching commands', async t => {
  ...
  assert.deepEqual((await readdir(path.join(app.root,'.devreview'))).filter(file=>file.startsWith('browser-')),[]);
});
...
test('Chromium device preview: touch, rotation, desktop reset, review evidence and owned lifecycle', {
  ...
  const directories = (await readdir(path.join(app.root,'.devreview'))).filter(file=>file.startsWith('browser-'));
  assert.equal(directories.length,1);
  const [port,endpoint] = (await readFile(path.join(app.root,'.devreview',directories[0],'DevToolsActivePort'),'utf8')).split(/\r?\n/);
