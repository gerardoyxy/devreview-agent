...
export const binary=fileURLToPath(new URL(`../target/debug/nudgethis${process.platform==='win32'?'.exe':''}`,import.meta.url));
...
export async function git(root,...args){const r=await command('git',['-c','user.name=NudgeThis Test','-c','user.email=test@example.invalid','-c','commit.gpgsign=false',...args],{cwd:root});if(r.code)throw new Error(r.stderr);return r.stdout.trim();}
export async function fixture(t) { const root=await mkdtemp(path.join(tmpdir(),'nudgethis-test-'));t.cleanups=[];t.after(async()=>{for(const cleanup of t.cleanups.reverse())await cleanup();await rm(root,{recursive:true,force:true,maxRetries:10,retryDelay:100});});await git(root,'init','-b','main');await writeFile(path.join(root,'.gitignore'),'.nudgethis/\n');await writeFile(path.join(root,'button.css'),'.button { color: red; }\n');await writeFile(path.join(root,'other.txt'),'unchanged\n');await git(root,'add','.');await git(root,'commit','-m','Fixture');return root; }
...
export async function configure(root,{agents=[{id:'fixture',mode:'workflow',flavor:'edit'}],commands=[],workers=2,validationTimeout=5000}={}) {
 ...
 await writeFile(path.join(root,'nudgethis.toml'),lines.join('\n')+'\n');await git(root,'add','nudgethis.toml');await git(root,'commit','-m','Configure test');
}
export async function start(t,root,options) {
 ...
 return {root,url,token,headers,raw,api,stop,child,closed,get errors(){return errors;},worktree:id=>path.join(root,'.nudgethis','worktrees',id)};
}
