...
export async function git(root, ...args) {
  const result = await run('git', ['-c', 'user.name=NudgeThis Test', '-c', 'user.email=test@example.invalid', '-c', 'commit.gpgsign=false', ...args], { cwd: root });
  if (result.code) throw new Error(result.stderr);
  return result.stdout.trim();
}
export async function fixture(t) {
  const root = await mkdtemp(path.join(tmpdir(), 'nudgethis-test-'));
  t.cleanups = [];
  t.after(async () => {
