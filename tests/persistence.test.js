...
test('configuration rejects wildcard / external origins', async t => {
  const root = await fixture(t);
  await writeFile(path.join(root, 'devreview.config.mjs'), "export default {server: {allowedOrigins: ['https://example.com']}};");
...
test('local credentials are not created unless the state directory is ignored', async t => {
  ...
  await assert.rejects(createCore({ root, config, agent }), /Ignore .devreview/);
  const { access } = await import('node:fs/promises');
  await assert.rejects(access(path.join(root, '.devreview/token')));
});
