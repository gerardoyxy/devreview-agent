...
test('legacy SQLite is backed up, migrated once, and interrupted tasks recover as failed',async t=>{
 const root=await fixture(t);await configure(root);await mkdir(path.join(root,'.nudgethis'));const file=path.join(root,'.nudgethis','tasks.sqlite');const db=new DatabaseSync(file);db.exec('CREATE TABLE tasks(id INTEGER PRIMARY KEY AUTOINCREMENT,data TEXT NOT NULL)');
 ...
 assert.equal((await readdir(path.join(root,'.nudgethis'))).filter(f=>f.startsWith('tasks.before-rust-')).length,1);app=await start(t,root,false);assert.equal((await app.api('/api/tasks/QA-1')).messages.length,1);await app.stop();assert.equal((await readdir(path.join(root,'.nudgethis'))).filter(f=>f.startsWith('tasks.before-rust-')).length,1);
});
test('reject/retry preserve earlier versions; delete removes messages and keeps audit',async t=>{
 ...
 await app.api(`/api/tasks/${task.id}`,undefined,{method:'DELETE'});assert.equal((await app.raw(`/api/tasks/${task.id}`)).status,404);await app.stop();const db=new DatabaseSync(path.join(app.root,'.nudgethis','tasks.sqlite'));assert.equal(db.prepare('SELECT COUNT(*) n FROM messages').get().n,0);assert.equal(db.prepare("SELECT COUNT(*) n FROM audit WHERE action='deleted'").get().n,1);db.close();
});
test('credentials require ignored state, and state symlinks are refused',async t=>{
 const root=await fixture(t);await configure(root);await writeFile(path.join(root,'.gitignore'),'');const result=await command(binary,['--root',root,'start']);assert.notEqual(result.code,0);assert.match(result.stderr,/Ignore .nudgethis/);await assert.rejects(access(path.join(root,'.nudgethis','token')));
 if(process.platform!=='win32'){await writeFile(path.join(root,'.gitignore'),'.nudgethis/\n');await symlink(path.join(root,'other.txt'),path.join(root,'.nudgethis','token'));const r=await command(binary,['--root',root,'start']);assert.match(r.stderr,/symlinks/);assert.equal(await readFile(path.join(root,'other.txt'),'utf8'),'unchanged\n');}
});
test('legacy executable configuration and external origins are explicitly refused',async t=>{
 const root=await fixture(t);await writeFile(path.join(root,'nudgethis.config.mjs'),'throw new Error("must not execute")');let r=await command(binary,['--root',root,'start']);assert.match(r.stderr,/Legacy nudgethis.config.mjs/);await writeFile(path.join(root,'nudgethis.toml'),'[server]\nallowedOrigins=["https://example.com"]\n');r=await command(binary,['--root',root,'start']);assert.match(r.stderr,/loopback origins/);
});
...
test('appearance persists, publishes SSE, and rejects CSS injection and malformed fonts',async t=>{
 ...
 for(const bad of [{...value,mode:'x'},{...value,fonts:{...value.fonts,body:'url(https://evil.example)'}},{...value,customFonts:[{name:'NudgeThisFontbody',data:Buffer.from('not a font').toString('base64')}]},{...value,palettes:{...value.palettes,dark:{...palette,accent:'red; background:url(x)'}}}])assert.equal((await app.raw('/api/appearance',bad)).status,400);
