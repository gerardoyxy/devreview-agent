...
test('selection controls in Chromium: gestures, no accidental actions, shortcuts, touch and live preferences', {
  ...
  await page.waitForFunction(() => document.querySelector('[data-nudgethis-overlay]').shadowRoot.querySelector('.pick-launcher').title.startsWith('Shift + Left click'));
  ...
  const update = async (button, keyboard = { code: 'KeyE', modifiers: [] }, modifiers = []) => {
    ...
    await page.waitForFunction(button => document.querySelector('[data-nudgethis-overlay]').shadowRoot.querySelector('.pick-launcher').title.startsWith(button === null ? 'Mouse gesture off' : ['Left click', 'Middle click', 'Right click', 'Side button: Back', 'Side button: Forward'][button]), button);
  };
  ...
  await page.reload(); await init(page); await page.waitForFunction(() => document.querySelector('[data-nudgethis-overlay]').shadowRoot.querySelector('.pick-launcher').title.startsWith('Mouse gesture off'));
  ...
  await phone.locator('[data-nudgethis-overlay] .panel').waitFor(); assert.equal(new URL(phone.url()).hash, '');
  assert.deepEqual(await phone.evaluate(() => window.actions), []);
  await phone.locator('[data-nudgethis-overlay] .panel').getByRole('button', { name: 'Close', exact: true }).tap();
  ...
  await page.waitForFunction(() => document.querySelector('[data-nudgethis-overlay]').shadowRoot.querySelector('.pick-launcher').title.startsWith('Alt / Option + Right click'));
